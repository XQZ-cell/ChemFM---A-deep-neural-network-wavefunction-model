...
class DenseBlock(kfac_jax.DenseTwoKroneckerFactored):
    ...
    def update_curvature_matrix_estimate(
        ...
        if not kfac_jax.utils.first_dim_is_size(batch_size, x, dy):
            ...
            x, dy = (
                jnp.tile(a[None], (batch_size, *(1 for _ in a.shape))).reshape((-1, a.shape[-1]))
                for a in (x, dy)
            )
...
def make_graph_patterns():
    r"""Create graph patterns for the KFAC optimizer."""
    custom_patterns = []
    for n_extra_dims in range(4, 0, -1):
        for with_bias in (True, False):
            custom_patterns.append(
                make_dense_pattern(with_bias, extra_dims=tuple(8 + i for i in range(n_extra_dims)))
            )
            kfac_jax.set_default_tag_to_block_ctor(
                f"repeated{n_extra_dims}_dense", RepeatedDenseBlock
            )

    kfac_jax.set_default_tag_to_block_ctor("dense", DenseBlock)

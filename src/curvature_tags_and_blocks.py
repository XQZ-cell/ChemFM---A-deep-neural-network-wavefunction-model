...
_repeated_dense3 = jax.vmap(_repeated_dense2, in_axes=[0, [None, None]])
_repeated_dense1_no_b = jax.vmap(_dense, in_axes=[0, [None]])
_repeated_dense2_no_b = jax.vmap(_repeated_dense1_no_b, in_axes=[0, [None]])
_repeated_dense3_no_b = jax.vmap(_repeated_dense2_no_b, in_axes=[0, [None]])
...
repeated_dense3_with_bias_pattern = kfac_jax.tag_graph_matcher.GraphPattern(
    name="repeated_dense3_with_bias",
    tag_primitive=kfac_jax.layers_and_loss_tags.layer_tag,
    compute_func=_repeated_dense3,
    parameters_extractor_func=_repeated_dense_parameter_extractor,
    example_args=[np.zeros([7, 8, 9, 11, 13]), [np.zeros([13, 7]), np.zeros([7])]],
)

repeated_dense3_no_bias_pattern = kfac_jax.tag_graph_matcher.GraphPattern(
    name="repeated_dense3_no_bias",
    tag_primitive=kfac_jax.layers_and_loss_tags.layer_tag,
    compute_func=_repeated_dense3_no_b,
    parameters_extractor_func=_repeated_dense_parameter_extractor,
    example_args=[np.zeros([7, 8, 9, 11, 13]), [np.zeros([13, 7])]],
)

GRAPH_PATTERNS = (
    repeated_dense1_with_bias_pattern,
    repeated_dense2_with_bias_pattern,
    repeated_dense3_with_bias_pattern,
    repeated_dense1_no_bias_pattern,
    repeated_dense2_no_bias_pattern,
    repeated_dense3_no_bias_pattern,

...
class PsiformerLayer(nn.Module):
    ...
    def __call__(
        ...
        attn_out = self.attention(x_norm, x_norm, x_norm, edges)
        if self.use_gate:
            # Gated attention
...
class PsiformerOrbitals(nn.Module):
    ...
    def setup(self):
        ...
        if self.envelope == 'simple':
            self.envelope_fn = envelopes.IsotropicEnvelope(natoms, num_orbs_per_e)
        else:
            raise NotImplementedError(f"Unknown envelope type: {self.envelope}.")
    ...
    def __call__(self, pos, spins, atoms, charges) -> jax.Array:
        ...
        ae_features, _ = make_features(
            ae, ee, r_ae, r_ee, self.nspins, self.rescale_inputs)
        
        # Input linear layer
        ae_features = self.input_linear(ae_features)  # Shape: (nelectron, dim_mlp_hidden)
        
        # Psiformer layers
        for psiformer_layer in self.psiformer_layers:
            if self.use_edge_bias:
                ae_features = psiformer_layer(ae_features, r_ee.squeeze(-1))
            else:
                ae_features = psiformer_layer(ae_features, None)
...
class Psiformer(nn.Module):
    ...
    def setup(self):
        if self.act_fn == 'tanh':
            activation_fun = jax.nn.tanh
        else:
            raise NotImplementedError(f"Unknown activation function: {self.act_fn}.")
